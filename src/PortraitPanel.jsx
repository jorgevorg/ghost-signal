...
const iconFilter = (color, distorted = false) => {
  ...
  return distorted ? `${base} blur(1.5px)` : base;
};

// ── Static interference — unknown signal display ───────────────────────────
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: MONO, fontSize: 6, color, letterSpacing: 2,
          animation: 'signalBlink 1.1s step-end infinite', textAlign: 'center', lineHeight: 1.8 }}>
          ⬡ SIGNAL{'\n'}LOST ⬡
        </div>
      </div>
    </div>
...
function QuipBubble({ text, color, loading }) {
  ...
  return (
    <div style={{
      width: '100%', minHeight: 28, padding: '4px 5px',
...
export default function PortraitPanel({
  ...
  // Lock portrait choice per enemy — don't re-roll on re-render
  ...
  // Pick portrait once per enemy id
  ...
  // Distortion: active when enemy selected but comms not open
  const distorted = hasEnemy && !commsOpen;
  ...
  return (
    <div style={{ width: 116, flexShrink: 0, background: '#03060a',
      ...
      <style>{`
        ...
        @keyframes portraitFadeIn { from{opacity:0;transform:translateY(4px)} to{opacity:1;transform:translateY(0)} }
        ...
        @keyframes glitchShift {
          ...
          24%      { transform:skewX(0deg) translateX(0);   filter:var(--gf) brightness(0.7); }
      ...
      {hasEnemy ? (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4,
          ...
          <div style={{ fontFamily:MONO, fontSize:5, color:`${eColor}55`, letterSpacing:2,
            ...
            {commsOpen ? faction : '⬡ CONNECTING ⬡'}
          </div>

          {/* Portrait frame */}
          <div style={{ width:54, height:54, position:'relative',
            border:`1px solid ${eColor}${commsOpen ? '55' : '22'}`,
            borderRadius:3, background:`${eColor}06`, overflow:'hidden',
            ...
            {portrait ? (
              <>
                <img src={portrait} alt={faction} style={{
                  ...
                  '--gf': iconFilter(eColor),
                  filter: iconFilter(eColor, distorted),
                  animation: distorted ? 'glitchShift 1.8s ease-in-out infinite' : 'none',
                  transition: 'filter 0.4s',
                }} />
                {/* Scanline overlay while distorted */}
                {distorted && (
                  <div style={{ position:'absolute', inset:0, pointerEvents:'none',
                    background:'repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(0,0,0,0.28) 3px,rgba(0,0,0,0.28) 4px)',
                    animation:'scanlines 0.18s linear infinite', zIndex:2 }} />
                )}
              </>
            ) : (
              <StaticNoise color={eColor} size={54} />
            )}
          </div>

          {/* HP badge */}
          <div style={{ fontFamily:MONO, fontSize:6, letterSpacing:1,
            color:eColor, opacity:0.75, textAlign:'center' }}>
