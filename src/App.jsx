...
function CyberTerminal(props){
  ...
  return React.createElement("div",{style:{flex:1,display:"flex",flexDirection:"column",minHeight:0,background:"#020c02",borderTop:"1px solid "+CB_GREEN+"2a"}},
    ...
    React.createElement("div",{ref:scrollRef,style:{flex:1,overflowY:"auto",padding:"8px 12px",fontFamily:MONO,fontSize:13,lineHeight:1.7,letterSpacing:.2,minHeight:0}},
      ...
      logs.map(function(l,i){return React.createElement("div",{key:i,style:{color:lineColor(l.t),animation:"termScroll .12s ease",textShadow:lineGlow(l.t)}},...(typeof l.s==="string"?(function(){var parts=l.s.split(/(rol\s+d\d+)/gi);if(parts.length===1)return [l.s];return parts.map(function(part,pi){var m=part.match(/^rol\s+d(\d+)$/i);if(!m)return part;var sides=parseInt(m[1],10);var clr=lineColor(l.t)||"#FF0066";return React.createElement("span",{key:pi,onClick:function(){var roll;if(sides===66){var t=Math.floor(Math.random()*6)+1;var u=Math.floor(Math.random()*6)+1;roll=t*10+u;}else{roll=Math.floor(Math.random()*sides)+1;}var note="[OS ROLL: d"+sides+" -> "+roll+"]";setLogs(function(prev){return prev.map(function(e,ei){return ei===i?{t:e.t,s:e.s.replace(part,"d"+sides+" -> "+roll)}:e;}).concat([{t:"system",s:note}]);});},style:{display:"inline-block",fontFamily:MONO,fontSize:"0.82em",letterSpacing:2,color:clr,border:"1px solid "+clr,borderRadius:2,padding:"1px 7px",margin:"0 3px",cursor:"pointer",verticalAlign:"middle",userSelect:"none",background:clr+"16"}},part);});})():[l.s]));}),
